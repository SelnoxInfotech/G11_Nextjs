...
async function generateSitemap() {
  try {
    ...
    if (rssXml && rssXml.length > 0) {
      rssXml.forEach((url) => {
        feed.item({
          title: `Dream11 today match prediction for ${url.description}.Win big with accurate tips & best Dream11 team prediction, Today Dream11 Team Check out!` ,
          ...
          url: `https://g11prediction.com/cricket-match-predictions/${modifyString(url.url1 + "-dream11 prediction today match , dream 11 prediction , Fantasy Cricket Tips, Playing XI, Pitch Report, Injury Update")}/${url.id}/`,
        });
      });
      ...
      <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ${rssXml.map((data) => `
          <url>
            <loc>https://g11prediction.com/cricket-match-predictions/${modifyString(data.url1 + "-dream11 prediction today match , dream 11 prediction , Fantasy Cricket Tips, Playing XI, Pitch Report, Injury Update")}/${data.id}/</loc>
