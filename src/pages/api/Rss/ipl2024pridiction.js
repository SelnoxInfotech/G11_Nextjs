...
export async function generateRssXml() {
    const data = await axios.get(`https://g11fantasy.com/NewsSection/FilterbySubCategory/11`);
    const rssData = data.data.data;
    const sitemapTeamguide = `<?xml version="1.0" encoding="UTF-8"?>
          <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
             ${rssData.map((url) => `
                       <url>
                        <loc>https://g11prediction.com/ipl-2024-dream11-predictions/${modifystr(url.Title)}/${url.id}/</loc>
