...
export async function generateRssXml() {
    const data = await axios.get(`https://www.g11fantasy.com/NewsSection/Get-VideoNews/`);
    const rssData = data.data.data;
    const sitemapTeamguide = `<?xml version="1.0" encoding="UTF-8"?>
          <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
             ${rssData.map((url) => `
                       <url>
                        <loc>https://g11prediction.com/latest-video/${url.Title.replace(/\s+/g, '-').slice(0, -1).toLowerCase() }/${url.id}/</loc>
