                ...
                <div className={`col-lg-8 col-12`}>
                    {l?.map((data, index) => (
                        ...
                        <React.Fragment key={index}>
                            ...
                            />
                            <p className={`${style.bredcrumlong} mb-0`}><span onClick={()=>router.replace(`/`)}>Home</span>{" > "}<span onClick={()=>router.replace(`/${router.query.dynamicslug}`)}>{capitalizeAndRemoveHyphens(router.query.dynamicslug)}</span>{" > "}<span className={style.activeRoute}>{capitalizeAndRemoveHyphens(data.Title)}</span></p>
